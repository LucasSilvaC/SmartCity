...
export default function Table({ sensores = [], limit = 10 }) {
  ...
  return (
    <section
      className="h-full w-[40%] flex flex-col rounded-sm shadow-xl overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900"
      aria-label="Tabela de sensores conectados"
    >
        ...
        <table className="w-full h-full table-fixed">
          <thead className="bg-gradient-to-r from-green-700 via-[#126b4b] to-green-700">
            <tr>
              <th
                scope="col"
                className="w-1/3 px-4 py-3 text-left text-white font-semibold uppercase tracking-wide"
              >
                Unidade
              </th>
              <th
          ...
          <tbody>
            ...
            {sensoresLimitados.map((sensor, idx) => (
              <tr
                key={sensor.id}
                className={`${idx % 2 === 0 ? "bg-black/60" : "bg-black/30"
              ...
              >
                ...
                <td
                  className={`px-4 py-3 font-semibold ${sensor.status ? "text-green-400" : "text-red-400"
                    }`}
                >
